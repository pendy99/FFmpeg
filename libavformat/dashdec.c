    ...
    if (c->is_live) {
        if (pls->n_fragments) {
            ...
            if (pls->presentation_timeoffset) {
                num = pls->first_seq_no + (((get_current_time_in_sec() - c->availability_start_time) - c->suggested_presentation_delay - c->period_start) * pls->fragment_timescale) / pls->fragment_duration - c->min_buffer_time;
...
static const AVOption dash_options[] = {
    ...
    { "decryption_key", "Media decryption key (hex)", OFFSET(cenc_decryption_key), AV_OPT_TYPE_STRING, {.str = NULL}, INT_MIN, INT_MAX, .flags = FLAGS },
    {NULL}
};
